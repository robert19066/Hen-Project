...
import textwrap
import threading
from deep_translator import GoogleTranslator, DeeplTranslator, MyMemoryTranslator, LingueeTranslator
...
def select_model():
    ...
    for idx, model in enumerate(models, 1):
        ...
        icon = "[AI MODEL]" if caps.get('reasoning') else "[REASONING MODEL]" if caps.get('code_specialized') else "[CODE MODEL]"
...
def run_chat():
    ...
    # Translator selection (default: GoogleTranslator)
    ...
    input_lang = 'ro'
    output_lang = 'en'
    # Language and service selection
    def select_language():
        nonlocal input_lang, output_lang, translator_service
        console.print("\n[bold cyan]Available languages:[/bold cyan] en, ro, fr, de, es, it, ru, zh, ja, ...")
        in_lang = input(f"Input language (default: {input_lang}): ").strip() or input_lang
        out_lang = input(f"Output language (default: {output_lang}): ").strip() or output_lang
        console.print("[bold cyan]Available services:[/bold cyan] google, deepl, mymemory, linguee")
        service = input("Translator service (default: google): ").strip().lower() or 'google'
        if service in supported_services:
            translator_service = supported_services[service]
        else:
            console.print(f"[warning]Unknown service, using default (google).[/warning]")
            translator_service = GoogleTranslator
        input_lang = in_lang
        output_lang = out_lang
    # Optionally call select_language() here or expose via a command
    conversation_history = []
    console.print(Panel(f"[bold green]Connected to {model_name}[/bold green]\n[dim]Ctrl+R: Toggle Romanian | Ctrl+L: Change Language/Service | Type 'exit' to quit[/dim]", border_style="green"))
    while True:
        try:
            ...
            if ROMANIAN_MODE:
                with console.status("[dim]Translating...[/dim]"):
                    try:
                        user_msg = translator_service(source=input_lang, target=output_lang).translate(user_msg)
                    except Exception as e:
                        console.print(f"[warning]Translation error: {e}[/warning]")
            ...
            if ROMANIAN_MODE:
                try:
                    response = translator_service(source=output_lang, target=input_lang).translate(response)
                except Exception as e:
                    response = f"[Translation Error] {response}"
            # Render Markdown
